...
function [state_ref, input_ref] = conRef_preview_off(controller,system)
    % --- STATE REFERENCE CALCULATION--- %
    if isempty(controller.Model)
        state_ref = kron(ones(1,controller.N),[controller.reference,controller.reference]);
    else
        state_ref = kron(ones(1,controller.N),[controller.reference/controller.Model.gamma,controller.reference]);
    end
    ...
    system.ref=controller.reference;
end
